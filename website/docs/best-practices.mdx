...
Programs with simple configuration options most likely only need a single config-file, or not even that if the config is defined in the program's main file. As the configurations become more complex, it's better to set up a system to organize the config-files.
...
Different developers often want to configure their programs slightly different from one another. Pushing those configurations to Git will become a headache of merge conflicts and it's tedious to apply your configuration changes every time you switch to a new branch.

To overcome these limitations, many systems make use of user configs that override certain keys in the default config. This can easily be implemented in Anyfig. 

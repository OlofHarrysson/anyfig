...
## Using the Config
Once the config object is initialized, the config-values can be accessed via the dot notation. Most people pass the config-object around their code, accessing the config-values at various places to initilize their other components.

### Global Config
Anyfig offers an alternative to passing the config-object around. The anyfig.setup_config function registers the config-object with Anyfig, allowing it to be accessed from any file with the anyfig.cfg() function. 

 Global objects are often considered bad practice and is thus _not recommended_ but often very practical. One could argue that the problem is allevited with Anyfig as **config-objects are immutable** after initialization by default.
...
class SomeClass():
  def __init__(self):
    ...
    # ... Use config-values to initialize SomeClass
```

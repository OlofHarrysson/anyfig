...
To avoid unintended behaviours its good practice to validate the config. This can be done by checking that the config-values are the correct types and within a subset of allowed values. Anyfig offers this functionality though the field function.
